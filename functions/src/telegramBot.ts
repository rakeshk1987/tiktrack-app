...
async function saveSession(telegramUserId: number, session: Omit<TelegramSession, 'updated_at' | 'expires_at'>) {
  await db.collection('telegram_sessions').doc(String(telegramUserId)).set({
    ...session,
...
async function requireMiniAppLink(req: functions.https.Request, res: functions.Response): Promise<{ user: TelegramUser; link: TelegramLink } | null> {
...
export const telegramMiniAppBootstrap = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const programsByChild: Record<string, ProgramOption[]> = {};
...
export const telegramMiniAppCreateSchedule = functions.https.onRequest(async (req, res) => {
  if (setCors(req, res)) return;
  if (req.method !== 'POST') {
    res.status(405).json({ ok: false, error: 'Method not allowed.' });
    return;
  ...
  try {
    ...
    const draft = req.body?.draft as ScheduleDraft | undefined;
    if (!draft?.childId || !draft.scheduleType || !draft.title || !draft.startAt || !draft.endAt) {
    ...
    if (duplicateId && !req.body?.createAnyway) {
...
export const telegramMiniAppListToday = functions.https.onRequest(async (req, res) => {
  if (setCors(req, res)) return;
  if (req.method !== 'POST') {
    res.status(405).json({ ok: false, error: 'Method not allowed.' });
    return;
...
export const telegramMiniAppListWeek = functions.https.onRequest(async (req, res) => {
  if (setCors(req, res)) return;
  if (req.method !== 'POST') {
    res.status(405).json({ ok: false, error: 'Method not allowed.' });
    return;
...
export const telegramMiniAppDeleteSchedule = functions.https.onRequest(async (req, res) => {
  if (setCors(req, res)) return;
  if (req.method !== 'POST') {
    res.status(405).json({ ok: false, error: 'Method not allowed.' });
    return;
  ...
  try {
    ...
    if (!auth) return;
    const childId = String(req.body?.childId || '');
    const collection = String(req.body?.collection || '') as ScheduleItem['collection'];
...
async function handleDetailsText(chatId: number, telegramUserId: number, session: TelegramSession, text: string) {
  ...
  if (!draft.startAt) {
    await saveSession(telegramUserId, { ...session, step: 'await_details', draft });
  ...
  const nextSession = { ...session, step: 'confirm_create' as SessionStep, draft };
  ...
  await sendMessage(
    chatId,
    `${draftSummary(draft)}${duplicateWarning}`,
    [
 ...
 */
export const onApprovalCreated = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    const approvalType = String(data.type || 'task') as 'task' | 'routine' | 'exam' | 'custom';
