...
exports.generateSmartDailyTasks = exports.generateMotivationalTasks = exports.generateChallengeTasks = exports.generateExamPrepTasks = exports.generateDailyTasks = exports.generateTasksForSlot = exports.DEFAULT_TASK_RULES = void 0;
/**
 * Default task generation rules mapped to routine categories
 */
exports.DEFAULT_TASK_RULES = {
    study: {
 ...
 */
const generateTasksForSlot = (slot, profile, weakSubjects = [], upcomingExams = [], currentMood) => {
    const rule = exports.DEFAULT_TASK_RULES[slot.category || 'leisure'];
    if (!rule) {
        return [];
    }
...
exports.generateTasksForSlot = generateTasksForSlot;
/**
 * Generate full day's tasks based on routine
 */
const generateDailyTasks = (routineSlots, profile, exams, upcomingEvents, currentMood, maxTasksPerDay = 10) => {
    ...
    for (const slot of routineSlots) {
        const slotTasks = (0, exports.generateTasksForSlot)(slot, profile, weakSubjects, upcomingExams, currentMood);
 ...
 */
const generateExamPrepTasks = (exam, daysUntilExam, profile) => {
    const examTasks = [];
    const subject = exam.title;
    // Intensity increases as exam gets closer
    let taskCount = 1;
    if (daysUntilExam <= 1)
        taskCount = 4; // Final day push
    else if (daysUntilExam <= 3)
        taskCount = 3; // 3 days before
    else if (daysUntilExam <= 7)
        taskCount = 2; // 1 week before
...
exports.generateExamPrepTasks = generateExamPrepTasks;
/**
 * Generate challenge tasks (harder, for mastery-level children)
 */
const generateChallengeTasks = (profile, weeklyCompletionRate) => {
...
exports.generateChallengeTasks = generateChallengeTasks;
/**
 * Generate motivational tasks for low performers
 */
const generateMotivationalTasks = (profile, mood) => {
    if ((profile.consistency_score === undefined || profile.consistency_score > 60) &&
...
exports.generateMotivationalTasks = generateMotivationalTasks;
/**
...
const generateSmartDailyTasks = (routineSlots, profile, exams, upcomingEvents, currentMood, weeklyCompletionRate = 50) => {
    ...
    if (upcomingExams.length > 0) {
        const examTasks = (0, exports.generateExamPrepTasks)(upcomingExams[0].event, upcomingExams[0].daysUntil, profile);
        tasks = tasks.concat(examTasks);
    }
    // Add challenge tasks for top performers
    const challengeTasks = (0, exports.generateChallengeTasks)(profile, weeklyCompletionRate);
