    ...
    <div className="mt-6 space-y-5">
      <div className={clsx('rounded-[1.75rem] border p-4 shadow-[0_18px_45px_rgba(0,0,0,0.16)]', panelClass)}>
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => setProfileTab('settings')} className={clsx('rounded-xl px-4 py-2 text-sm font-bold', profileTab === 'settings' ? 'bg-cyan-500 text-white' : 'border border-white/20')}>Profile Settings</button>
          <button type="button" onClick={() => setProfileTab('special-dates')} className={clsx('rounded-xl px-4 py-2 text-sm font-bold', profileTab === 'special-dates' ? 'bg-fuchsia-500 text-white' : 'border border-white/20')}>Special Dates</button>
          <button type="button" onClick={() => setProfileTab('growth')} className={clsx('rounded-xl px-4 py-2 text-sm font-bold', profileTab === 'growth' ? 'bg-emerald-500 text-white' : 'border border-white/20')}>Growth</button>
        </div>
      </div>
      ...
      {profileTab === 'settings' && (
        <div className={clsx('rounded-[1.75rem] border p-5 shadow-[0_18px_45px_rgba(0,0,0,0.16)]', panelClass)}>
          ...
          <p className={clsx('mt-1 text-sm', mutedTextClass)}>Your identity, interests, and growth journey in one place.</p>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div className={clsx('rounded-[1.1rem] border px-4 py-3', isDark ? 'border-white/10 bg-white/6' : 'border-indigo-200/70 bg-white/80')}>
              <p className={clsx('text-[11px] font-bold uppercase tracking-[0.14em]', accentCaptionClass)}>Avatar</p>
              <p className="mt-1 text-2xl font-bold">{avatarEmoji}</p>
            </div>
            <div className={clsx('rounded-[1.1rem] border px-4 py-3', isDark ? 'border-white/10 bg-white/6' : 'border-indigo-200/70 bg-white/80')}>
              <p className={clsx('text-[11px] font-bold uppercase tracking-[0.14em]', accentCaptionClass)}>Age</p>
              <p className="mt-1 text-base font-bold">{age} years</p>
            </div>
            <div className={clsx('rounded-[1.1rem] border px-4 py-3', isDark ? 'border-white/10 bg-white/6' : 'border-indigo-200/70 bg-white/80')}>
              <p className={clsx('text-[11px] font-bold uppercase tracking-[0.14em]', accentCaptionClass)}>Level</p>
              <p className="mt-1 text-base font-bold">{level.levelName}</p>
            </div>
            <div className={clsx('rounded-[1.1rem] border px-4 py-3', isDark ? 'border-white/10 bg-white/6' : 'border-indigo-200/70 bg-white/80')}>
              <p className={clsx('text-[11px] font-bold uppercase tracking-[0.14em]', accentCaptionClass)}>Stars to Next</p>
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-2">
            <div className={clsx('rounded-[1.3rem] border px-4 py-4', isDark ? 'border-white/10 bg-white/6' : 'border-indigo-200/70 bg-white/80')}>
              <p className={clsx('text-xs font-bold uppercase tracking-[0.18em]', accentCaptionClass)}>Identity</p>
              <input value={petName} onChange={(event) => setPetName(event.target.value)} placeholder="Pet name" className="mt-3 w-full rounded-xl border border-white/20 bg-white/5 px-3 py-2 text-sm" />
              <div className="mt-3 flex flex-wrap gap-2">
                {AVATARS.map((emoji) => (
                  <button key={emoji} type="button" onClick={() => setAvatarEmoji(emoji)} className={clsx('rounded-xl border px-3 py-1.5 text-xl', avatarEmoji === emoji ? 'border-cyan-300 bg-cyan-500/15' : 'border-white/20')}>{emoji}</button>
                ))}
              </div>
              <input value={interests} onChange={(event) => setInterests(event.target.value)} placeholder="Interests (comma separated)" className="mt-3 w-full rounded-xl border border-white/20 bg-white/5 px-3 py-2 text-sm" />
            ...
            <div className={clsx('rounded-[1.3rem] border px-4 py-4', isDark ? 'border-white/10 bg-white/6' : 'border-indigo-200/70 bg-white/80')}>
              ...
              <input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder="New password (min 6 chars)" className="mt-3 w-full rounded-xl border border-white/20 bg-white/5 px-3 py-2 text-sm" />
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => void requestParentReset()} className="rounded-xl border border-white/20 px-4 py-2 text-sm font-bold">Request Parent Reset</button>
              </div>
            </div>
          </div>
        </div>
